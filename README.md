...
- табличный редактор маршрутов соединений с динамическим числом шагов;
...
6. `Run WireViz` в той же фоновой очереди сохраняет YAML в выбранный каталог и
   создаёт SVG, HTML, PNG и таблицу BOM.

Кнопка `Refresh Preview` выполняет валидацию сразу, а рендер запускает в фоне.
Во время работы виден индикатор. Все задачи WireViz выполняются последовательно;
...
1. Слева — дерево проекта с группами `Разъёмы`, `Кабели`, `Наконечники` и
   `Соединения`.
...
- `Создать шлейф` — построить последовательную разводку;
- `Обновить предпросмотр` — обновить YAML, выполнить проверки и перерисовать SVG.

Горячие клавиши: `Ctrl+N` — новый проект, `Ctrl+O` — открыть, `Ctrl+S` —
сохранить, `Ctrl+Shift+S` — сохранить как. Несохранённые изменения отмечаются
...
разъёмов, кабелей и наконечников, а также структурированные шаги соединений. Используйте его для
...
Текущая версия схемы — `schema_version: 2`. Файлы без этого поля считаются
версией 0, а строковые маршруты v1 преобразуются в структурированные шаги;
обе версии автоматически мигрируют при открытии и при следующем сохранении
...
- нет undo/redo и автосохранения;
