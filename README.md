...
Пустые поля показывают пример допустимого ввода. Наведение раскрывает подробную
подсказку: формат CSV, соответствие позиции номеру жилы/контакта, единицы длины
и сечения, назначение BOM-полей. Для цветов приведена легенда кодов WireViz:
`BK`, `BN`, `BU`, `GN`, `GY`, `OG`, `PK`, `RD`, `VT`, `WH`, `YE`, `GNYE`.
